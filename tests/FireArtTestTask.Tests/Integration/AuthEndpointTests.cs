...
public class AuthEndpointTests : IClassFixture<CustomWebApplicationFactory>
{
    ...
    {
        var request = new SignupRequest("test@example.com", "password123");
    ...
    {
        var request = new SignupRequest("duplicate@example.com", "password123");
    ...
    {
        var request = new SignupRequest("not-an-email", "password123");
    ...
    {
        var signup = new SignupRequest("login-test@example.com", "password123");
    ...
    {
        var signup = new SignupRequest("wrong-pw@example.com", "password123");
        await _client.PostAsJsonAsync("/api/auth/signup", signup);

        var login = new LoginRequest("wrong-pw@example.com", "wrongpassword");
        var response = await _client.PostAsJsonAsync("/api/auth/login", login);
    ...
    {
        var login = new LoginRequest("nonexistent@example.com", "password123");
    ...
    {
        var signup = new SignupRequest("forgot@example.com", "password123");
        await _client.PostAsJsonAsync("/api/auth/signup", signup);

        var request = new ForgotPasswordRequest("forgot@example.com");
